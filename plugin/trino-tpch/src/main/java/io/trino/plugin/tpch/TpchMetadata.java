        ...
        implements ConnectorMetadata
{
    ...
    @Inject
    public TpchMetadata(TpchConfig config, JsonMapper mapper, ConnectorExpressionEvaluator connectorExpressionEvaluator)
    {
        this(mapper, config.getColumnNaming(), config.getDecimalTypeMapping(), config.isPredicatePushdownEnabled(), config.isPartitioningEnabled(), Optional.ofNullable(config.getTableScanRedirectionCatalog()), Optional.ofNullable(config.getTableScanRedirectionSchema()), connectorExpressionEvaluator);
    }
    ...
    private Set<NullableValue> filterValues(Set<NullableValue> nullableValues, TpchColumn<?> column, Constraint constraint, ConnectorExpressionEvaluator.Prepared prepared)
    {
        ...
        return nullableValues.stream()
                ...
                .filter(value -> !Boolean.FALSE.equals(prepared.evaluate(ImmutableMap.of(columnHandle.columnName(), value)).orElse(null)))
                .collect(toSet());
    }
