...
public final class HiveWriteUtils
{
    ...
    public static List<String> createPartitionValues(List<Type> partitionColumnTypes, Page partitionColumns, int position)
    {
        ...
        for (int field = 0; field < partitionColumns.getChannelCount(); field++) {
            // TODO: FixME
            String value = toPartitionValue(null, partitionColumnTypes.get(field), partitionColumns.getBlock(field), position, Optional.empty());
    ...
    public static List<String> createPartitionValues(List<String> partitionNames, List<Type> partitionColumnTypes, Page partitionColumns, int position, Optional<PartitionProjection> partitionProjection)
    {
        ImmutableList.Builder<String> partitionValues = ImmutableList.builder();
        for (int field = 0; field < partitionColumns.getChannelCount(); field++) {
    ...
    private static String toPartitionValue(String name, Type type, Block block, int position, Optional<PartitionProjection> partitionProjection)
    {
        ...
        if (TIMESTAMP_MILLIS.equals(type)) {
            ...
            Optional<String> value = partitionProjection.flatMap(projection -> projection.toPartitionValue(name, epochMicros));
            if (value.isPresent()) {
                return value.get();
            }
